...
public class GuiNewChat extends Gui
{
    ...
    public void printChatMessageWithOptionalDeletion(String par1Str, int par2)
    {
        ...
        if (antispam)
        {
            for (int i = 0; i < 5; ++i)
            {
                ...
                try
                {
                    cl = ((ChatLine) this.chatLines.get(i));
                    line = cl.getChatLineString();
                }
                catch (Exception e)
                {
                    continue;
                ...
                try
                {
                    int index = line.lastIndexOf(ChatColor.RED + " x");
                    count = Integer.parseInt(line.substring(index + 4, line.length())) + 1;
                    line = line.substring(0, index);
                }
                catch (Exception e) {}

                if (line.equals(par1Str))
                {
                    ChatLine ncl = new ChatLine(this.mc.ingameGUI.getUpdateCounter(), line + ChatColor.RED + " x" + count, par2);
                    this.chatLines.remove(i);
                    this.chatLines.add(0, ncl);
                    isAdded = true;
                }
            }
        }
    ...
    private void func_96129_a(String par1Str, int par2, int par3, boolean par4)
    {
        ...
        if (region.parseLine(par1Str))
        {
            if (isRegionOwnerActivityCheckActivated)
            {
                if (region.isOk())
                {
                    for (String owner : region.owners)
                    {
                        mc.thePlayer.sendChatMessage("/seen " + owner);
                    }

                    region = new WGRegion();
                    isRegionOwnerActivityCheckActivated = false;
                }

                if (!par1Str.contains("Region: "))
                {
                    return;
                }
            }
            ...
            if (region.isOk())
            {
                if (par1Str.contains("Bounds: "))
                {
                    par1Str += " " + region.getBounds();
                }
        ...
        while (this.field_96134_d.size() > maxChatLines)
        {
