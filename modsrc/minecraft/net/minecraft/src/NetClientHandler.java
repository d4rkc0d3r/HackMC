...
import d4rk.mc.event.TimeUpdateEvent;
...
public class NetClientHandler extends NetHandler
{
    ...
    public void addToSendQueue(Packet par1Packet)
    {
        if (!this.disconnected)
        {
            if (par1Packet == null)
            {
                Hack.logWarning("NetClientHandler.addToSendQueue(null)");
                return;
            }

            if (EventManager.fireEvent(new PreSendPacketEvent(par1Packet)))
            {
                return;
            }
            ...
            {
                if (CommandManager.parseLocal(((Packet3Chat)par1Packet).message))
                {
                    return;
                }

                if (EventManager.fireEvent(new PlayerChatEvent(((Packet3Chat)par1Packet).message)))
                {
                    return;
                }
            }

            if (par1Packet.getPacketId() == 13)
            {
                Hack.somePlayerAction();
            }
            ...
            if (false)
            {
                switch (par1Packet.getPacketId())
                {
                    ...
                    default:
                        Hack.log("Sending Packet: " + par1Packet.toString());
                        break;
                }

                if (par1Packet instanceof Packet15Place && false)
                {
                    Packet15Place p = (Packet15Place)par1Packet;
                    System.out.println("p.getDirection(): " + p.getDirection());
                    System.out.println("p.getXOffset():   " + p.getXOffset());
                    System.out.println("p.getYOffset():   " + p.getYOffset());
                    System.out.println("p.getZOffset():   " + p.getZOffset());
                    System.out.println("p.getXPosition(): " + p.getXPosition());
                    System.out.println("p.getYPosition(): " + p.getYPosition());
                    System.out.println("p.getZPosition(): " + p.getZPosition());
                    System.out.println("p.getItemStack(): " + p.getItemStack());
                }
                else if (par1Packet instanceof Packet14BlockDig && false)
                {
                	Packet14BlockDig p = (Packet14BlockDig)par1Packet;
                    System.out.println("p.status: " + p.status);
                }
                else if (par1Packet instanceof Packet102WindowClick && false)
                {
                    Packet102WindowClick p = (Packet102WindowClick)par1Packet;
                    System.out.println("p.window_Id:     " + p.window_Id);
                    System.out.println("p.inventorySlot: " + p.inventorySlot);
                    System.out.println("p.mouseClick:    " + p.mouseClick);
                    System.out.println("p.itemStack:     " + String.valueOf(p.itemStack));
                    System.out.println("p.action:        " + p.action);
                    System.out.println("p.holdingShift:  " + p.holdingShift);
                }
                else if (par1Packet instanceof Packet101CloseWindow && false)
                {
                    Packet101CloseWindow p = (Packet101CloseWindow)par1Packet;
                    System.out.println("p.window_Id:     " + p.windowId);
                }
            }

            this.netManager.addToSendQueue(par1Packet);
            EventManager.fireEvent(new PostSendPacketEvent(par1Packet));
        }
    }
    ...
    {
        ChatEvent event = new ChatEvent(ChatMessageComponent.func_111078_c(par1Packet3Chat.message).func_111068_a(true));
        ...
        if (!ImproveChat.onReceiveMsg(event.message))
        {
    ...
    {
        if (EventManager.fireEvent(new TimeUpdateEvent(par1Packet4UpdateTime)))
        {
            return;
        }

        this.mc.theWorld.func_82738_a(par1Packet4UpdateTime.worldAge);
        Hack.handleUpdateTime(par1Packet4UpdateTime.time);
    }
    ...
    public void handleRespawn(Packet9Respawn par1Packet9Respawn)
    {
        ...
        Hack.handleRespawn(par1Packet9Respawn);
    }
