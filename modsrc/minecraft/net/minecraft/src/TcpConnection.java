            ...
            if (var2 != null)
            {
                ...
                if (!this.isServerTerminating)
                {
                    if (var2.canProcessAsync() && this.theNetHandler.canProcessPacketsAsync())
                    {
                        ...
                        if (!d4rk.mc.event.EventManager.fireEvent(new d4rk.mc.event.PreProcessPacketEvent(var2, true)))
                        {
                            var2.processPacket(this.theNetHandler);
                            d4rk.mc.event.EventManager.fireEvent(new d4rk.mc.event.PostProcessPacketEvent(var2, true));
                        }
                    }
                    else
                    {
