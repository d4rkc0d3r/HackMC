...
public class Quarry {
	...
	public Quarry(BlockWrapper from, BlockWrapper to) {
		...
		if(from.world != to.world) throw new QuarryException("Quarry worlds differ!");
		this.start();
	}
	...
	public Quarry(BlockWrapper sign) {
		...
		if(s.blockType.blockID == Block.signPost.blockID) throw new QuarryException("Sign must be a wall sign!");
		if(!s.signText[0].equalsIgnoreCase("[Quarry]")) throw new QuarryException("Sign is no valid Quarry sign!");
		...
		case BlockWrapper.POSZ: 
			...
			to.x -= isLeft ? -lr : lr;
		break;
		default: throw new QuarryException("Sign has unexpected metadata!");
		}
...
//		to.setIDandMetadata(Block.cloth.blockID, 5); // lime wool
	...
	private BlockWrapper[] calculateStaircaise() {
		...
		for(int y=from.y-1;y>=to.y;y--) {
			...
			for(int i=0;;i++) {
				...
				if(tmp.isInsideOf(from, to)) break;
				dir+=2;
				if(dir==6) dir = 3;
				else if(dir==7) dir = 2;
				if(i>4) dir = BlockWrapper.POSY;
			}
			last = tmp;
			result.add(tmp);
		}
	...
	public BlockWrapper getCurrent() {
		try { return content[current].clone(); }
