...
public class CommandManager {
	...
	static public boolean parse(String cmd, String sender, String rank) { // returns true if it found the command
		...
		if(cmd.startsWith("/setdefault ") && Permission.has(sender, rank, Permission.LOCALE)) {
			...
			if(args!=null) if(args.length>1) perm = Integer.parseInt(args[1]);
	...
	static public boolean isInputMessageHidden(String str) {
		if(str.startsWith("/tps")) return true;
		if(str.startsWith("/ctps")) return true;
		if(str.startsWith("/status")) return true;
		if(str.startsWith("/send")) return true;
		if(str.startsWith("/focus")) return true;
		if(str.startsWith("/setdefault")) return true;
		return false;
	}
	
	static public boolean isOutputMessageHidden(String str) {
		if(str.startsWith("Current Server TPS: ")) return true;
		if(str.startsWith("Current Client TPS: ")) return true;
		if(str.startsWith("Health:")) return true;
		return false;
	}
